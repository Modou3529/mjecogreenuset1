...
async function callGeminiChat(messages,systemInstruction=''){
  if(!geminiKey){openApiModal();throw new Error('No API key.');}
  // Filter to only user/assistant messages, ensure starts with user
...
async function doChat(){
  ...
  try{
    ...
    console.error('Technical details:',e);
    if(!e.message.includes('API key'))
      chatHist.push({role:'assistant',content:'Eco Green is currently updating. Please try again in a moment! 🌿'});
...
async function ideaToChat(i){
  ...
  try{
    ...
    if(!e.message.includes('API key'))
      chatHist.push({role:'assistant',content:'Eco Green is currently updating. Please try again in a moment! 🌿'});console.error('Technical details:',e);
  }finally{chatBusy=false;renderChat(false);}
}
